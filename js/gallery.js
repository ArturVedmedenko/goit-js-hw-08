...
const imageCardsTemlate = images.map(createImageCardTemplate).join(``);

const imagesGalleryEl = document.querySelector(`.gallery`);

imagesGalleryEl.innerHTML = imageCardsTemlate;

const onImageCardClick = event => {
  event.preventDefault();

  const clickedEl = event.target;
  if (!clickedEl.classList.contains(`gallery-image`)) {
    return;
  }

  const imageSrc = clickedEl.dataset.source;
  const imageAlt = clickedEl.alt;

  const modalInstance = basicLightbox.create(`<img src="${imageSrc}"
  alt="${imageAlt}" />
  `);
